...
ParameterPool::ParameterPool() {
	for(int i = 0; i < NUMBER_OF_VOICES; i++) {
		store(p_ADSR1_Attack, i, "adsr1_attack", 2200.0, 2200.0);
		store(p_ADSR1_Decay, i, "adsr1_decay", 2200.0, 2200.0);
		store(p_ADSR1_Sustain, i, "adsr1_sustain", 1.0, 1.0);
		store(p_ADSR1_Release, i, "adsr1_release", 2200.0, 2200.0);

		store(p_ADSR2_Attack, i, "adsr2_attack", 2200.0, 2200.0);
		store(p_ADSR2_Decay, i, "adsr2_decay", 2200.0, 2200.0);
		...
		store(p_WS1_Detune, i, "ws1_detune", 430.0, 430.0);
		...
		store(p_WS2_Transpose, i, "ws2_transpose", 0.0, 0.0);
		
		store(p_KS_Amount, i, "ks_amount", 1.0, 0.0);
		store(p_KS_DelayTime, i, "ks_delay", 440.0, 440.0);
		store(p_KS_Feedback, i, "ks_feedback", 0.999999, 0.999999);
		store(p_KS_FilterCutoff, i, "ks_dampening", 10000.0, 10000.0);
		store(p_KS_Transpose, i, "ks_transpose", 0.0, 0.0);

		store(p_LFO1_Rate, i, "lfo1_rate", 1.0, 1.0);
